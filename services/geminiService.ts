...
import { AnalysisResponse, MedicationResponse, TriageLevel, HealthRecord, HealthInsight, FirstAidGuide, LANGUAGES, DoctorListing } from "../types";
...
export const analyzeSymptoms = async (
  ...
  2. DETAIL LEVEL: **EXTREMELY VERBOSE**. 
     ...
     - 'recommendedSpecialist': IDENTIFY the single most appropriate medical specialty to treat this condition (e.g., 'Cardiologist', 'Dermatologist', 'Orthopedic').
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            recommendedSpecialist: { type: Type.STRING, description: "The specific type of doctor recommended (e.g., 'Dermatologist') in target language." },
            visualAnalysis: {
              type: Type.OBJECT,
              properties: {
...
/**
 * Find Nearby Doctors using Google Maps Grounding
 */
export const findNearbyDoctors = async (
  ...
  // Prompt explicitly requests nearest doctors without rating bias
  // Robustness change: explicitly asking for "medical professionals suitable for..." handles conditions (e.g., Flu) and titles (e.g., Dentist)
  const prompt = `
  Find 5 medical professionals, clinics, or hospitals suitable for treating "${specialty}" closest to "${location}" using Google Maps.
  
  OUTPUT INSTRUCTION:
  After finding the doctors, extract their details and return them in a strict JSON array format inside a markdown block.
  
  CRITICAL SORTING RULE:
  - Sort results by PROXIMITY (Distance).
  - Do NOT prioritize or filter based on rating/stars. 
  - I want the absolute closest doctors, regardless of their rating.
  ...
  LANGUAGE: Content in ${targetLanguage}.
  
  Example Output:
  \`\`\`json
  [
    {"name": "Dr. Smith", "rating": "4.9", "address": "123 Main St", "phone": "555-0100", "lat": 40.71, "lng": -74.01},
    ...
  ]
  \`\`\`
  `;
  ...
  try {
    const response = await ai.models.generateContent({
      model: FAST_MODEL, // Using Flash model for reliable tool support
      contents: prompt,
      config: {
        tools: [{ googleMaps: {} }],
        toolConfig: center ? {
    ...
    if (response.text) {
       let jsonStr = "";
       // Robust parsing: Try to find code block first
       const jsonMatch = response.text.match(/```(?:json)?\s*(\[[\s\S]*?\])\s*```/);
       if (jsonMatch && jsonMatch[1]) {
          jsonStr = jsonMatch[1];
       } else {
          // Fallback: Try to find the array directly
          const start = response.text.indexOf('[');
          const end = response.text.lastIndexOf(']');
          if (start !== -1 && end !== -1) {
             jsonStr = response.text.substring(start, end + 1);
          }
       }

       if (jsonStr) {
          try {
             const rawData = JSON.parse(jsonStr) as any[];
             // Sanitization map to ensure types
             return rawData.map(d => ({
                ...
                lng: d.lng ? parseFloat(d.lng) : undefined
             })) as DoctorListing[];
          } catch (e) {
             console.error("Failed to parse doctor JSON", e);
          }
       }
    }
    return [];
    ...
    // Return empty array instead of throwing to prevent UI crash, allow retry
    return [];
  }
};