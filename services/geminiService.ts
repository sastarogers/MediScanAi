...
// Using Gemini 3 Pro for advanced reasoning and multimodal capabilities
...
export const analyzeSymptoms = async (
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: { parts },
      config: {
        // Use advanced thinking for complex diagnostic tasks
...
export const scanMedication = async (
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        // We use a small thinking budget for med identification accuracy, but keep it lower than diagnostics
