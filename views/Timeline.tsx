    ...
    <PageTransition>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 mb-2">
          <div className="flex items-center justify-between bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm p-2 pr-2 rounded-2xl border border-white/20 shadow-sm animate-fade-in">
            <div className="flex items-center gap-3 pl-2">
              <button onClick={onBack} className="p-2 hover:bg-white dark:hover:bg-slate-800 rounded-full transition-colors shadow-sm"><ArrowLeft className="w-5 h-5" /></button>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('health_dashboard', language)}</h2>
            </div>
            <div className="flex gap-2">
              ...
              >
                ...
                <span className="hidden sm:inline ml-2">{t('ai_insights', language)}</span>
              </Button>
              ...
              <Button onClick={() => setIsAddingEntry(true)} className="py-2 px-3 text-sm h-10"><Plus className="w-5 h-5" /> {t('add', language)}</Button>
            </div>
          </div>
          
          {/* Search and Filter Bar */}
          <div className="glass-panel p-2 rounded-2xl flex flex-col md:flex-row gap-2 animate-fade-in" style={{ animationDelay: '0.1s' }}>
            <div className="relative flex-1 group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
              <input type="text" placeholder={t('search', language)} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl border-none bg-slate-50/50 dark:bg-slate-800/50 text-sm focus:ring-2 focus:ring-blue-500/50 outline-none transition-all" />
            </div>
            <div className="relative">
              <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full md:w-auto pl-4 pr-10 py-2.5 rounded-xl border-none bg-slate-50/50 dark:bg-slate-800/50 text-sm outline-none cursor-pointer focus:ring-2 focus:ring-blue-500/50 appearance-none font-medium">
                <option value="all">{t('all_types', language)}</option>
                <option value="symptom">{t('symptoms', language)}</option>
                <option value="medication">{t('medications', language)}</option>
                <option value="appointment">{t('appointments', language)}</option>
              </select>
        ...
        <div className="animate-slide-up duration-500 pb-20">
          {history.length === 0 ? (
            <div className="text-center py-20 text-slate-400">
              ...
              <p className="text-lg font-medium">{t('no_records', language)}</p>
              <Button variant="outline" onClick={() => setIsAddingEntry(true)} className="mt-4">{t('add_first', language)}</Button>
            </div>
          ) : (
                    ...
                    <Card className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-0 overflow-hidden cursor-pointer transition-all duration-300 ${isExpanded ? 'ring-2 ring-blue-500 shadow-xl z-10 scale-[1.02]' : 'hover:shadow-lg hover:-translate-y-0.5'}`} onClick={() => toggleExpand(record.id)}>
                      ...
                      <div className={`bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 transition-all duration-500 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="p-4 space-y-4">
                          ...
                          {record.notes && <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700"><p className="text-xs font-bold text-slate-500 uppercase mb-1">{t('my_notes', language)}</p><p className="text-sm">{record.notes}</p></div>}
                          <div className="flex gap-2 pt-2">
                            <button onClick={(e) => { e.stopPropagation(); openEdit(record); }} className="flex-1 py-2 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-sm font-medium transition-colors flex items-center justify-center gap-2 active:scale-95"><Edit2 className="w-4 h-4" /> {t('edit', language)}</button>
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(record.id); }} className="flex-1 py-2 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-900/50 text-sm font-medium transition-colors flex items-center justify-center gap-2 active:scale-95"><Trash2 className="w-4 h-4" /> {t('delete', language)}</button>
                          </div>
                        </div>
                      </div>
                    </Card>
        ...
        {/* AI INSIGHTS MODAL */}
        <Modal isOpen={showInsightsModal} onClose={() => setShowInsightsModal(false)} title={t('dr_ai', language)}>
          {insights ? (
            <div className="space-y-6">
              <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                <div className="flex items-center gap-2 mb-2 text-indigo-800 dark:text-indigo-300">
                   <Sparkles className="w-5 h-5" />
                   <h3 className="font-bold">{t('medical_summary', language) || "Medical Summary"}</h3>
                </div>
                <p className="text-slate-700 dark:text-slate-300 leading-relaxed text-sm">{insights.summary}</p>
                <p className="text-xs text-slate-400 mt-2 text-right">{t('analysis_generated', language)}</p>
              </div>

              {insights.patterns.length > 0 && (
                <div>
                  <h4 className="font-bold mb-3 flex items-center gap-2 text-slate-900 dark:text-white"><TrendingUp className="w-5 h-5 text-blue-500" /> {t('detected_patterns', language)}</h4>
              ...
              {insights.recommendations.length > 0 && (
                <div>
                  <h4 className="font-bold mb-3 flex items-center gap-2 text-slate-900 dark:text-white"><Stethoscope className="w-5 h-5 text-emerald-500" /> {t('recommendations', language)}</h4>
        ...
        <Modal isOpen={isAddingEntry || !!isEditingId} onClose={() => { setIsAddingEntry(false); setIsEditingId(null); }} title={isEditingId ? t('edit_entry', language) : t('add_new_entry', language)}>
           <div className="space-y-4">
            <input className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-blue-500" placeholder={t('summary', language)} value={newEntry.summary} onChange={e => setNewEntry({...newEntry, summary: e.target.value})} />
            <textarea className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-blue-500" placeholder={t('details', language)} value={newEntry.details} onChange={e => setNewEntry({...newEntry, details: e.target.value})} rows={4} />
            <Button onClick={isEditingId ? handleEditSubmit : handleAddSubmit} className="w-full">{t('save', language)}</Button>
           </div>
        </Modal>
        
        <Modal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} title={t('print', language)}><Button onClick={() => window.print()}>{t('print', language)}</Button></Modal>
      </div>
    </PageTransition>
  );
};
